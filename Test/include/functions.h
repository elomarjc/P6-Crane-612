...
int pathAtoB (float xPos, float yPos, float xContainer, float yContainer){
    if (step==0) {    
        ...
        if (0.20<xPos && xPos<0.40 && 0.35<yPos && yPos<0.60){ //put the right values
            step=1;
    ...
    if (step==1) {
        Setpoint_x = 0.5;
        if (0.40>xPos || xPos>0.55 || 0.40>xContainer || xContainer>0.55) {    //If trolley is not above container. pm 2 cm
           failTime = millis();
    ...
    if (step==2) {
        ...
        if (yPos<1.17) {
            failTime = millis();
        } else if (millis() > failTime + 400) {  //400
            Serial3.println("M1"); 
            magnet_sw = 1;
            step=3;
        }
    ...
    if (step==4) {
        ...
        } else if (millis() > failTime + 6000) {     //This can be changed to something as a function of velocity and position
            step=5;   
