...
void setup()
{
  ...
  thetaPID.SetSampleTime(10); //Angle sampling rate is 10x times faster than xPID
...
void loop()
{
  ...
  double tempVar = Output_x - Output_theta;

  analogWrite(pin_pwm_x, tempVar);

  Serial.println("PID Input_x: " + String(Input_x) + ", PID Setpoint_x: " + String(Setpoint_x) + ", PID Output_x: " + String(Output_x) + ", Angle: " + String(Input_theta) + ", output_theta: " + String(Output_theta));
}